  ...
  context('World of Doomens test cases', async function () {
    ...
    describe('Mint function with proof', async function () {
      it('Can not mint token if proof is invalid', async function () {
        await expect(this.wod.connect(this.addr1).mint(this.ownerProof,1,{value:ethers.utils.parseEther("0.05")}))
      ...
      it('Can not mint token more than limit', async function () {
        await expect(this.wod.mint(this.ownerProof, 6, {value: ethers.utils.parseEther("0.3")}))
        .to.be.revertedWith('Can not mint token more than limit');
      });

      it('Can not mint token if ether is not enough', async function () {
        await expect(this.wod.mint(this.ownerProof, 1, {value: ethers.utils.parseEther("0.01")}))
        .to.be.revertedWith('Invalid amount of ether to buy token!');
      });

      it('Requires mint amount to be greater than 0', async function () {
        await expect(this.wod.mint(this.ownerProof, 0, { value: ethers.utils.parseEther("0.05") }))
        .to.be.revertedWith(
